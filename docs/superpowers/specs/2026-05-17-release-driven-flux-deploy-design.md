...
- `services/{monolith,frontend}/kubernetes/base/deployment.yaml` (修正): Setters annotation を削除 (`:latest` は維持、Flux で書き換える対象から外す)
...
`:latest` は維持し、Setters annotation (`# {"$imagepolicy": ...}`) のみ削除する。base は「Flux で書き換える対象から外す」のが目的で、image tag のデフォルト値としては `:latest` を保つ。production overlay が必ずこの image 行を strategic merge patch で上書きするため、production cluster には `:latest` は流れ込まない。base 単独で kustomize build した結果 (= production overlay 経由しない使い方) は `:latest` を返す。
...
- `services/{monolith,frontend}/kubernetes/base/deployment.yaml` の Setters annotation を削除 (`:latest` は維持)
...
| `base/deployment.yaml` の `:latest` が production cluster に流れ込む | production overlay が必ず deployment patch で `:vX.Y.Z` に上書きするため流れ込まない。Setters annotation も overlay 側にあり、Flux は overlay のみ書き換え対象 |
