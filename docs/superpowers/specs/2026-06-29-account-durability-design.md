...
| media object storage | DB 行は cascade delete、**物理 file は孤児で残置** (MVP scope 外、運用 cleanup は将来の別 sub-project) |
...
| Media | `media__media_files` (uploader_account_id = self) — DB 行のみ delete。**物理 object storage file は残置** (孤児、運用 cleanup) |
