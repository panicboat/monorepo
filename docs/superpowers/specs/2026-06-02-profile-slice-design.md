...
- **出勤予定（shift schedule）**: rx-sns の cast profile に表示あり。但しスケジュール/商取引次元（keystone で plans/schedules/出勤を drop）に属し profile のフィールドではない。rx-sns に寄せて再導入する場合は **別の shifts/出勤スライス**として、booking 次元を再検討する時に扱う（profile スライス対象外）。
