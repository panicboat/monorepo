...
### `SendCode` 改訂
1. **rate limit**: 同 phone_number の過去 24h の sms_verifications を見て、過去 24h に 5 件以上なら拒否 (`SendCode::RateLimitError`)、最終送信が 60 秒以内なら拒否。判定は use_case 内に閉じ、閾値は定数 (`MIN_RESEND_INTERVAL_SECONDS` / `DAILY_SEND_LIMIT` / `DAILY_WINDOW_SECONDS`)。
2. **旧コード無効化**: 新コード作成前に、同 phone の未消費・未検証行の `expires_at` を `Time.now` に更新して即時 expire させる (行は残す＝送信回数 rate-limit の判定母集団として必要)。
