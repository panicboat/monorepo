...
### 5. リポジトリ設定変更 (Terragrunt 経由)

panicboat 配下のリポジトリ設定と branch protection は `platform/github/` 配下で Terragrunt 管理されている。`gh repo edit` や `gh api` を直接叩かず、`platform` リポの hcl ファイルを更新して通常の PR ベース apply フローで反映する。

#### マージ戦略の変更 (`platform/github/repository/modules/main.tf`)
...
これは Terragrunt module レベルの変更で、`var.repositories` に登録されている **全リポ (panicboat 配下 6 リポ全て)** に適用される。`ansible` / `dotfiles` も rebase merge ができなくなるが、個人運用のため運用上の影響はない。

**理由**: `release-please` は main の commit history を Conventional Commits としてパースする。rebase merge では PR の各 commit がそのまま main に入り、PR タイトル検証 (`semantic-pull-request`) では個別 commit message を保証できない → release-please が壊れる。Squash merge を強制すれば「PR タイトル = commit message」となり、release-please が正しく動く。

#### Required check の追加 (`platform/github/branch/envs/develop/<repo>.hcl`)

各リポの hcl ファイルで `branch_protection.main` の `required_status_checks` を上書き:
...
対象 4 リポ: `deploy-actions`, `panicboat-actions`, `monorepo`, `platform`

**タイミング上の制約**: Required に指定した check 名は、main で 1 度でも success していないと branch protection 側で「unknown check」状態になり PR がマージ不能になる。このため Required check 追加 (Phase 4) は Phase 1〜3b 完了後に実施する必要がある。

## Repository-by-repository plan

各リポへの workflow 追加は当該リポで実施。マージ戦略 / Required check は `platform` で集中管理 (Phase 0 / 4)。

| リポ | semantic-PR | lint-actions | renovate.json | release | App token 事前準備 | allowlist |
|---|---|---|---|---|---|---|
...
| `platform` (Phase 0) | — | — | — | — | — | `modules/main.tf` で `allow_rebase_merge = false` |
| `platform` (Phase 4) | — | — | — | — | — | 4 リポの `branch/envs/develop/*.hcl` で `required_status_checks` を追加 |
...
| Step | 内容 |
|---|---|
| 0.1 | `platform/github/repository/modules/main.tf` の `allow_rebase_merge = true` → `false` に変更（1 PR、1 行） |
| 0.2 | 通常の deploy-trigger workflow で terragrunt plan → main マージで apply |

これにより全リポで rebase merge が無効化され、Squash merge のみが許可される。
...
| Step | 内容 |
|---|---|
| 2.1 | `semantic-pull-request.yml` + `lint-actions.yml`（allowlist: `panicboat/panicboat-actions/`）+ Renovate `extends` 追加（1 PR） |
...
| Step | 内容 |
|---|---|
| 3.x.1 | `semantic-pull-request.yml` + `lint-actions.yml`（allowlist: `panicboat/panicboat-actions/`）+ Renovate `extends` 追加（1 PR） |
...
| Step | 内容 |
|---|---|
| 4.1 | `platform/github/branch/envs/develop/{deploy-actions,panicboat-actions,monorepo,platform}.hcl` の各々で `required_status_checks` に `["Validate PR title", "Ensure actions are pinned to SHAs"]` を追加（1 PR） |
| 4.2 | 通常の deploy-trigger workflow で terragrunt plan → main マージで apply |

Phase 1〜3b 完了後に実施 (各 check が main で 1 度成功している必要があるため)。

### 順序の制約

- **Phase 0** は Phase 1 開始前に完了させる (deploy-actions のマージ戦略が `release-please` の前提)
- **Phase 1** は Phase 3 より前に完了させる (`panicboat/deploy-actions/*` 参照を `@v1` に切り替えるため `v1.0.0` タグが必要)
- **Phase 2** は Phase 1 と独立、いつでも実施可
- **Phase 4** は Phase 1〜3b 全完了後に実施
...
# pinact v3 schema: name は正規表現、ref は必須
...
- [ ] `platform/github/repository/modules/main.tf` の `allow_rebase_merge = false`
- [ ] terragrunt apply 完了
- [ ] `gh repo view --json rebaseMergeAllowed` が `{"rebaseMergeAllowed":false}` を返す (任意の panicboat 配下リポで確認)
...
- [ ] `platform/github/branch/envs/develop/{deploy-actions,panicboat-actions,monorepo,platform}.hcl` で `required_status_checks` に 2 項目が追加されている
- [ ] terragrunt apply 完了
- [ ] 4 リポの `gh api repos/<owner>/<repo>/branches/main/protection/required_status_checks --jq '.contexts'` が `["Validate PR title", "Ensure actions are pinned to SHAs"]` を含む
...
| Squash merge 強制が既存の rebase merge 運用と衝突 | 既存の Renovate `automergeStrategy: squash` 設定と整合。手動 PR でも Squash のみとなるが、panicboat 配下は個人運用のため影響は限定的 |
