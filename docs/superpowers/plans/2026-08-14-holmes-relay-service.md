...
Note: `TestAlertmanagerHandler_Accepted` leaves `h.holmes` and `h.client` as `nil`; the goroutine it spawns will panic in the background, but the test only asserts the synchronous HTTP response (200), which is written before the goroutine runs. This is acceptable for this test's scope — Task 5 Step 3 wires real clients in `main.go`.
