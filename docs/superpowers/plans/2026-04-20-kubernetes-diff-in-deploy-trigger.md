...
**Architecture:** caller の `auto-label--deploy-trigger.yaml` が matrix を展開し、各 (service × environment) ごとに新規 reusable workflow `reusable--kubernetes-builder.yaml` を呼び出す。reusable 側で base/head を別ディレクトリに checkout → kustomize build → dyff between → sticky comment 投稿。`pull_request` イベントのみ。
...
| Path | 役割 |
|---|---|
| `.github/workflows/reusable--kubernetes-builder.yaml` (新規) | (service, environment) を受け取り、kustomize build → dyff between → PR コメントを実行する reusable workflow |
...
Run: `docker run --rm -v "$PWD:/repo" --workdir /repo rhysd/actionlint:latest .github/workflows/reusable--kubernetes-builder.yaml`
