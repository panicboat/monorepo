...
**Goal:** monolith と frontend の全コードを対象に、パターン統一・命名整理・技術的負債を解消する
...
**Files:**
- Create: `services/frontend/workspace/src/lib/grpc-errors.ts`
- Create: `services/frontend/workspace/src/lib/api-helpers.ts`

**Step 1: gRPC エラーコード定数を作成**

```typescript
// services/frontend/workspace/src/lib/grpc-errors.ts
...
git commit -m "feat(frontend): add gRPC error constants and API route helpers"
```
...
**Files:**
- Create: `services/frontend/workspace/src/modules/post/lib/api-mappers.ts`
- Modify: `services/frontend/workspace/src/app/api/guest/timeline/route.ts`
- Modify: `services/frontend/workspace/src/app/api/cast/timeline/route.ts`
- Modify: `services/frontend/workspace/src/app/api/feed/guest/route.ts`

**Step 1: Proto → JSON のサーバーサイドマッパーを作成**

```typescript
// services/frontend/workspace/src/modules/post/lib/api-mappers.ts
...
git commit -m "refactor(frontend): extract shared post API mapper, remove duplicate mapping in routes"
```
...
git commit -m "refactor(frontend): apply auth/error helpers to cast/timeline route"
```
...
Run: `cd services/frontend/workspace && pnpm lint && pnpm build`
Expected: No errors
...
```bash
git add src/app/api/
git commit -m "refactor(frontend): apply auth/error/pagination helpers across all API routes"
```
...
**Files:**
- Create: `services/frontend/workspace/src/modules/relationship/lib/mappers.ts`
- Modify: `services/frontend/workspace/src/app/api/guest/following/route.ts`
- Modify: `services/frontend/workspace/src/app/api/guest/favorites/route.ts`
- Modify: `services/frontend/workspace/src/app/api/guest/blocks/route.ts`
- Modify: `services/frontend/workspace/src/app/api/cast/blocks/route.ts`
...
git commit -m "refactor(frontend): extract relationship domain mappers from API routes"
```
...
**Files:**
- Create: `services/frontend/workspace/src/modules/trust/lib/mappers.ts`
- Modify: `services/frontend/workspace/src/app/api/me/trust/reviews/route.ts`
- Modify: `services/frontend/workspace/src/app/api/shared/trust/reviews/route.ts`（存在する場合）
...
git commit -m "refactor(frontend): extract trust domain mappers from API routes"
```
...
**Files:**
- Delete: `services/frontend/workspace/src/modules/feed/hooks/useGuestFeed.ts`
- Delete: `services/frontend/workspace/src/modules/feed/hooks/useCastFeed.ts`
- Modify: `services/frontend/workspace/src/modules/feed/hooks/index.ts`
...
git commit -m "refactor(frontend): remove dead code (useGuestFeed, useCastFeed)"
```
...
**Files:**
- Modify: `services/frontend/workspace/src/modules/post/hooks/useCastPosts.ts`
- Read: `services/frontend/workspace/src/modules/post/hooks/useTimeline.ts`（パターン参考）
...
git commit -m "refactor(frontend): migrate useCastPosts to usePaginatedFetch + authFetch"
```
...
**Files:**
- Modify: `services/frontend/workspace/src/modules/trust/hooks/useReviews.ts`
...
git commit -m "refactor(frontend): simplify useReviews to CRUD-only, use useInfiniteReviews for listing"
```
...
- Modify: `services/frontend/workspace/src/modules/portfolio/lib/cast/mappers.ts`
- Create or modify: `services/frontend/workspace/src/modules/portfolio/types.ts`（API レスポンス型を追加）
...
git commit -m "refactor(frontend): replace any types with proper interfaces in portfolio module"
```
...
| Phase 2 | Task 9-14 | Frontend: API ヘルパー、Post/Relationship/Trust mapper |
| Phase 3 | Task 15-18 | Frontend: デッドコード削除、フック移行、型修正 |
