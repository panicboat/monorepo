...
**Files:**
- Regenerate: `services/frontend/workspace/src/stub/portfolio/v1/cast_service_pb.ts`
...
```bash
grep -i "RANKING" services/frontend/workspace/src/stub/portfolio/v1/cast_service_pb.ts
```
...
```bash
git add services/frontend/workspace/src/stub/
git commit -m "chore(frontend): regenerate proto stubs after ranking removal"
```
...
**Files:**
- Modify: `services/frontend/workspace/src/app/api/guest/search/route.ts:11-22`

**Step 1: `parseStatusFilter` 関数からランキング case を削除**

`services/frontend/workspace/src/app/api/guest/search/route.ts` の `parseStatusFilter` 関数を以下に変更:
...
```bash
git add services/frontend/workspace/src/app/api/guest/search/route.ts
git commit -m "feat(frontend): remove ranking case from search API route"
```
...
`services/frontend/workspace/src/modules/portfolio/hooks/useInfiniteCasts.ts` を以下のように変更:
...
```bash
git add services/frontend/workspace/src/modules/portfolio/hooks/useInfiniteCasts.ts
git commit -m "feat(frontend): remove ranking from StatusFilter and add areaId to useInfiniteCasts"
```
...
```bash
cd services/frontend/workspace && pnpm build 2>&1 | tail -20
```
...
```bash
git add services/frontend/workspace/src/app/\(guest\)/search/SearchFilterOverlay.tsx
git commit -m "feat(frontend): remove ranking from overlay and add area filter section"
```
...
```bash
git add services/frontend/workspace/src/app/\(guest\)/search/page.tsx
git commit -m "feat(frontend): add area filter to search page and remove ranking from filter chips"
```
...
**Files:**
- Check: `services/frontend/workspace/src/modules/portfolio/hooks/index.ts`

**Step 1: hooks index で `useAreas` がエクスポートされているか確認**

`services/frontend/workspace/src/modules/portfolio/hooks/index.ts` を確認し、`useAreas` のエクスポートがなければ追加:
...
```bash
git add services/frontend/workspace/src/modules/portfolio/hooks/index.ts
git commit -m "chore(frontend): ensure useAreas is exported from hooks index"
```
