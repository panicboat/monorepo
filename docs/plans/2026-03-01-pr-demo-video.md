...
| Path Pattern | Impact |
|-------------|--------|
| `services/frontend/workspace/src/**` | フロントエンド直接変更 |
...
diff の内容から以下を特定:
1. **変更された画面**: `services/frontend/workspace/src/app/` のルート構造から対象ページを特定
...
UI_CHANGES=$(git diff main...HEAD --name-only 2>/dev/null | grep -E '^(services/frontend/workspace/src/|proto/|services/monolith/workspace/slices/.*/actions/)' | head -20)
...
```bash
git diff main...HEAD --name-only | grep -E '^(services/frontend/workspace/src/|proto/|services/monolith/workspace/slices/.*/actions/)'
```
