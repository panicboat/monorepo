...
Date: 2026-05-27 (Updated: 2026-05-29 — design tokens verified, 7 軸踏襲合意, Cast 評価共有 SNS placement 確定)
...
## Cast 評価共有 SNS — Placement Design (2026-05-29)

本プロダクトの差別化要素（収益化の要）。rx-sns.jp に参照パターン無しの独自設計。

### 機能モデル

- **形式**: DB 型（Guest 1 人 = 1 構造化レコード。客観事実 + 評価 + Cast 記入履歴）
- **透明性**: (b) 同意済み非常時開示型。登録時の規約同意で「評価情報が事業者・Cast 間で共有される」ことに同意取得 → 合法ルートに乗せる。通常 UI では Guest に評価を一切表示しない
- **収益化**: Cast の有料機能

### 法務前提（弁護士確認必須 — 最重要）

- 個人情報保護法・名誉毀損・要配慮個人情報の論点があり、設計確定前に個人情報保護/IT 法務に強い弁護士の確認が必須
- (b) は「同意取得型」で合法ルートを取るが、**開示請求導線を UI に出さず問い合わせ経由のみとする選択（後述 L3-4）は、「同意はしたが導線が無い」状態として同意有効性をさらに繊細にする**。この点が弁護士レビューの最重要ポイント
- 開示請求権（個情法 第33条）自体は設計で消せない。問い合わせ経由でも実際の開示・訂正・削除には必ず対応する運用が前提
...
- **法務観点（弁護士確認必須）**: 個人情報保護・名誉毀損・要配慮個人情報。特に (b) で開示導線を UI に出さない選択の同意有効性。design 確定前に弁護士レビュー
