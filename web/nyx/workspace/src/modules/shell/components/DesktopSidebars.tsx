...
const RECOMMENDED_CASTS = [
  ...
  { id: 102, name: "Rin", image: "https://api.dicebear.com/7.x/avataaars/svg?seed=Rin", tag: "Popular" },
  ...
  return (
    <aside className="hidden w-80 flex-col gap-6 pt-24 lg:flex sticky top-0 h-screen overflow-y-auto no-scrollbar">
      ...
      <div className="rounded-2xl border border-slate-100 bg-white p-6 shadow-sm">
        ...
        <div className="space-y-4">
          {RECOMMENDED_CASTS.map((cast) => (
            <Link href={`/cast/${cast.id}`} key={cast.id} className="flex items-center gap-4 group">
              ...
              <div className="flex-1">
                <div className="flex items-center justify-between">
                  <h4 className="font-bold text-slate-800 group-hover:text-pink-500 transition-colors">{cast.name}</h4>
                  <span className="text-[10px] bg-pink-50 text-pink-600 px-2 py-0.5 rounded-full font-bold">{cast.tag}</span>
                </div>
      ...
      <div className="rounded-2xl bg-gradient-to-br from-indigo-500 to-purple-600 p-6 text-white shadow-md">
        ...
        <p className="text-sm opacity-90 mb-4">Check our beginner's guide to the Ritual.</p>
