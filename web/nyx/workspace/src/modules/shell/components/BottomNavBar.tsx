    ...
    <div className="fixed bottom-0 left-0 right-0 z-50 mx-auto max-w-md border-t border-slate-100 bg-white/90 px-6 pb-safe pt-2 backdrop-blur-lg md:hidden">
      <div className="flex items-center justify-between">
        {tabs.map((tab) => {
          ...
          const isActive = pathname === tab.href || (tab.href !== "/" && tab.href !== "/manage" && pathname.startsWith(tab.href));

          return (
            <Link
              key={tab.id}
              href={tab.href}
              className={`flex flex-col items-center gap-1 p-2 transition-colors ${isActive ? activeColorClass : "text-slate-400 hover:text-slate-600"
                }`}
            >
