...
const MOCK_REVIEWS: Review[] = [
  {
    ...
    rating: 5,
    date: "2 days ago",
    comment: "Yuna was incredibly attentive and sweet. The conversation flowed naturally and I felt very relaxed.",
  ...
  },
  {
    ...
    rating: 4,
    date: "1 week ago",
    comment: "Great experience overall. Just wished the connection was a bit more stable at the start.",
    tags: ["Cute", "Funny"]
  },
  {
    id: "3",
...
export const ReviewList = () => {
  ...
  const displayedReviews = isExpanded ? [...MOCK_REVIEWS, ...EXTRA_MOCK_REVIEWS] : MOCK_REVIEWS;

  return (
    <div className="bg-slate-50 py-6">
      <div className="px-4 mb-4 flex items-center justify-between">
        <h3 className="font-serif text-lg font-bold text-slate-800">Recent Reviews</h3>
        <span className="text-sm text-pink-500 font-bold">4.8 <Star size={12} className="inline fill-pink-500" /> (124)</span>
      </div>
        ...
        {displayedReviews.map((review) => (
          <div key={review.id} className="rounded-xl border border-slate-100 bg-white p-4 shadow-sm">
            <div className="mb-2 flex items-center justify-between">
              <span className="text-xs font-bold text-slate-500">{review.userName}</span>
            ...
            <div className="flex gap-2">
              {review.tags.map(tag => (
                <span key={tag} className="rounded-md bg-slate-100 px-2 py-1 text-[10px] text-slate-500">
                  #{tag}
                </span>
              ))}
            </div>
