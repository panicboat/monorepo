...
## 2. Components Implementation (Domain Layer)
**デザイン方針**: 既存の Guest 側コンポーネントおよび Hime-Channel のスタイルを踏襲。新規デザインは行わず、既存の Tailwind クラスとレイアウトパターンを再利用する。

- [ ] 2.1 Create `src/modules/shell/components/cast/StatusToggle.tsx`:
...
- [ ] 2.2 Create `src/modules/trust/components/cast/EarningsSummary.tsx`:
...
- [ ] 2.3 Create `src/modules/ritual/components/cast/UpcomingReservations.tsx`:
  ...
  - Error state with retry button
  - **Style**: リスト形式、各アイテムは白背景カード。既存の ProfileCard や ReservationCard（あれば）のスタイルを参考
- [ ] 2.4 Create `src/modules/ritual/components/cast/ReservationListItem.tsx` (optional):
  - Single reservation item component (if needed for separation)
  - Props: `{ id, guestName, date, startTime, planName, status }`
  - Status badge with appropriate colors (confirmed: green, pending: yellow, completed: gray)
  - Date formatting: "YYYY-MM-DD HH:mm"
  - **Style**: 既存のカードコンポーネント（ProfileCard など）のレイアウトを踏襲。ステータスバッジは既存の badge スタイルを使用
...
- [ ] 4.1 Implement `src/app/(cast)/manage/dashboard/page.tsx`:
  - Onboarding パターンに従い、セクション配置のみを行う薄いレイヤーとして実装
  - Import `EarningsSummary` from `@/modules/trust/components/cast/EarningsSummary`
  - Import `UpcomingReservations` from `@/modules/ritual/components/cast/UpcomingReservations`
...
- [ ] 6.3 Verify `UpcomingReservations` component:
  ...
  - Error state shows retry button and works correctly
- [ ] 6.4 Verify `StatusToggle` in `CastTopNavBar`:
