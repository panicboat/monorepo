...
- **Frontend (Styling)**: デザイントークン未定義、カラー・スペーシング・タイポグラフィの散在
...
6. **デザイントークンの導入**
   - カラー、スペーシング、タイポグラフィを `config/theme.ts` に集約
   - ハードコードされた `text-[8px]`, `shadow-pink-200` 等を排除
   - ボタン・カード・バッジの統一コンポーネントを整備
