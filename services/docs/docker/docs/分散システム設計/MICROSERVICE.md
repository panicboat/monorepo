...
* **DB:** `reservations`, `cast_plans`
* **特性:** 絶対にデータをロストしてはいけない。ACID特性が重要。
* **なぜ分ける？:** ビジネスの核（金銭や信用の授受）であり、他の機能の変更によるバグの影響を避けるため。

#### 5. Trust Service (信用・評価)
...
sequenceDiagram
    ...
    participant Chat as Concierge SVC
    participant Book as Ritual SVC
...
* **障害分離:** チャットサーバーが落ちても、プロフィールは見れるし、過去の予約履歴（Ritual Svc）も確認できる。
* **開発速度:** 「Ritualチーム」は予約ロジックだけに集中し、「Portfolioチーム」はプロフィールのUI改善に集中できる。
...
**「Concierge (Chat)」とそれ以外** という2分割が、最初に検討すべき境界線になるでしょう。
