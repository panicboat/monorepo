...
RSpec.describe "Feed::UseCases::ListCastFeed", type: :database do
  ...
  before do
    # Create a cast (user_id is the PK, no separate id column)
  ...
  describe "#call" do
    ...
    it "returns all posts for the cast" do
      result = use_case.call(cast_user_id: cast_user_id, limit: 10)
    ...
    it "includes author information" do
      ...
      expect(result[:author].user_id).to eq(cast_user_id)
    end
    ...
    it "respects limit and returns has_more flag" do
      result = use_case.call(cast_user_id: cast_user_id, limit: 2)
    ...
    it "paginates correctly with cursor" do
      # First page
      page1 = use_case.call(cast_user_id: cast_user_id, limit: 2)
      expect(page1[:posts].size).to eq(2)

      # Second page
      page2 = use_case.call(cast_user_id: cast_user_id, limit: 2, cursor: page1[:next_cursor])
    ...
    context "when cast has no posts" do
      ...
      it "returns empty posts array" do
        result = use_case.call(cast_user_id: empty_cast_user_id, limit: 10)
