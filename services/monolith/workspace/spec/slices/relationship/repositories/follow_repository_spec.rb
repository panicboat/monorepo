...
RSpec.describe "Relationship::Repositories::FollowRepository", type: :database do
  ...
  describe "#follow" do
    it "creates an approved follow" do
      result = repo.follow(cast_id: cast_id, guest_id: guest_id)
  ...
  describe "#request_follow" do
    it "creates a pending follow request" do
      result = repo.request_follow(cast_id: cast_id, guest_id: guest_id)
      expect(result[:success]).to be true
  ...
  describe "#approve_follow" do
    it "approves a pending follow request" do
      repo.request_follow(cast_id: cast_id, guest_id: guest_id)
      result = repo.approve_follow(cast_id: cast_id, guest_id: guest_id)
  ...
  describe "#unfollow" do
    it "removes a follow" do
      repo.follow(cast_id: cast_id, guest_id: guest_id)
      result = repo.unfollow(cast_id: cast_id, guest_id: guest_id)
  ...
  describe "#following_cast_ids" do
    it "returns cast IDs the guest is following" do
      cast_id2 = SecureRandom.uuid
      repo.follow(cast_id: cast_id, guest_id: guest_id)
      repo.follow(cast_id: cast_id2, guest_id: guest_id)

      ids = repo.following_cast_ids(guest_id: guest_id)
  ...
  describe "#following_status_batch" do
    it "returns status for multiple casts" do
      ...
      repo.follow(cast_id: cast_id, guest_id: guest_id)
      repo.request_follow(cast_id: cast_id2, guest_id: guest_id)
  ...
  describe "#list_pending_requests" do
    it "returns pending follow requests" do
      3.times do |i|
        repo.request_follow(cast_id: cast_id, guest_id: SecureRandom.uuid)
      end
  ...
  describe "#pending_count" do
    it "returns count of pending requests" do
      2.times { repo.request_follow(cast_id: cast_id, guest_id: SecureRandom.uuid) }
      repo.follow(cast_id: cast_id, guest_id: SecureRandom.uuid)
