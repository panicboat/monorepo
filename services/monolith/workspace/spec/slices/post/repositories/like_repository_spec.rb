...
RSpec.describe "Post::Repositories::LikeRepository", type: :database do
  ...
  describe "#like" do
    it "creates a like" do
      repo.like(post_id: post.id, guest_user_id: guest_id)
      expect(repo.liked?(post_id: post.id, guest_user_id: guest_id)).to be true
    end
  ...
  describe "#unlike" do
    it "removes a like" do
      repo.like(post_id: post.id, guest_user_id: guest_id)
      repo.unlike(post_id: post.id, guest_user_id: guest_id)
      expect(repo.liked?(post_id: post.id, guest_user_id: guest_id)).to be false
    end
  end
  ...
  describe "#liked_status_batch" do
    it "returns liked status for multiple posts" do
      ...
      status = repo.liked_status_batch(post_ids: [post.id, post2.id], guest_user_id: guest_id)
