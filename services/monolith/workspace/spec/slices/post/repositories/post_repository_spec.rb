...
RSpec.describe "Post::Repositories::PostRepository", type: :database do
  ...
  describe "#create_post" do
    it "creates a post and returns it" do
      post = repo.create_post(cast_user_id: cast_id, content: "Hello world")
      expect(post.cast_user_id).to eq(cast_id)
  ...
  describe "#find_by_id_and_cast" do
    ...
    it "returns post when it belongs to cast" do
      post = repo.create_post(cast_user_id: cast_id, content: "Test")
      result = repo.find_by_id_and_cast(id: post.id, cast_user_id: cast_id)
      expect(result).not_to be_nil
  ...
  describe "#update_post" do
    it "updates post content" do
      post = repo.create_post(cast_user_id: cast_id, content: "Original")
  ...
  describe "#delete_post" do
    it "deletes the post" do
      post = repo.create_post(cast_user_id: cast_id, content: "To delete")
  ...
  describe "#save_media" do
    ...
    it "saves media with media_id for a post" do
      post = repo.create_post(cast_user_id: cast_id, content: "With media")
  ...
  describe "#list_by_cast_user_id" do
    ...
    it "limits results" do
      posts = repo.list_by_cast_user_id(cast_user_id: cast_id, limit: 2)
    ...
    it "does not return posts from other casts" do
      ...
      posts = repo.list_by_cast_user_id(cast_user_id: cast_id, limit: 10)
      expect(posts.map(&:cast_user_id).uniq).to eq([cast_id])
    end
  end
  ...
  describe "#list_all_visible" do
    ...
    it "excludes specified cast_ids" do
      posts = repo.list_all_visible(limit: 10, exclude_cast_user_ids: [cast1_id])
      expect(posts.map(&:cast_user_id)).not_to include(cast1_id)
    end
  end

  describe "#save_hashtags" do
    it "saves hashtags for a post" do
      post = repo.create_post(cast_user_id: cast_id, content: "Post with hashtags")
    ...
    it "replaces existing hashtags" do
      post = repo.create_post(cast_user_id: cast_id, content: "Post")
