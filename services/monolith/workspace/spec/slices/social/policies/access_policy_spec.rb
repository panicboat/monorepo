...
RSpec.describe Social::Policies::AccessPolicy do
  ...
  describe "#can_view_profile_details? (plans, schedules)" do
    context "public cast (Yuna)" do
      ...
      it "returns false when blocked" do
        allow(block_repo).to receive(:blocked?).and_return(true)
    ...
    context "private cast (Mio)" do
      ...
      it "returns false for pending follower" do
        allow(block_repo).to receive(:blocked?).and_return(false)
        allow(follow_repo).to receive(:following?).and_return(false) # pending != approved
      ...
      it "returns true for approved follower" do
        allow(block_repo).to receive(:blocked?).and_return(false)
        allow(follow_repo).to receive(:following?).and_return(true)
