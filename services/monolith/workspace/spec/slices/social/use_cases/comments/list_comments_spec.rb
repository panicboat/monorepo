  ...
  describe "#call" do
    it "returns comments with authors" do
      allow(comment_repo).to receive(:list_by_post_id)
        .with(post_id: post_id, limit: 20, cursor: nil)
    ...
    it "respects limit parameter" do
      allow(comment_repo).to receive(:list_by_post_id)
        .with(post_id: post_id, limit: 5, cursor: nil)
        .and_return([comment1])
    ...
    it "clamps limit to MAX_LIMIT" do
      allow(comment_repo).to receive(:list_by_post_id)
        .with(post_id: post_id, limit: 50, cursor: nil)
        .and_return([])
    ...
    it "clamps limit to minimum of 1" do
      allow(comment_repo).to receive(:list_by_post_id)
        .with(post_id: post_id, limit: 1, cursor: nil)
        .and_return([])
    ...
    it "sets has_more to true when more results exist" do
      ...
      allow(comment_repo).to receive(:list_by_post_id)
        .with(post_id: post_id, limit: 2, cursor: nil)
        .and_return(comments)
    ...
    it "decodes and passes cursor to repository" do
      ...
      allow(comment_repo).to receive(:list_by_post_id)
        .with(post_id: post_id, limit: 20, cursor: hash_including(:created_at, :id))
        .and_return([])
    ...
    it "loads guest author information" do
      ...
      expect(author[:image_url]).to eq("https://cdn.nyx.place/avatars/taro.jpg")
    end
  end
end
