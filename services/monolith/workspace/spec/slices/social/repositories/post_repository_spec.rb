...
RSpec.describe "Social::Repositories::PostRepository", type: :database do
  let(:repo) { Hanami.app.slices[:social]["repositories.post_repository"] }
  ...
  describe "#save_media" do
    it "saves media for a post" do
      ...
      result = repo.find_by_id(post.id)
      expect(result.cast_post_media.size).to eq(2)
      expect(result.cast_post_media.map(&:media_type)).to contain_exactly("image", "video")
    end
    ...
    it "replaces existing media" do
      ...
      result = repo.find_by_id(post.id)
      expect(result.cast_post_media.size).to eq(1)
      expect(result.cast_post_media.first.media_type).to eq("video")
    end
  end
