...
RSpec.describe Profile::Policies::ProfileAccessPolicy do
  let(:policy) { described_class.new }
  let(:follow_adapter) { instance_double(Profile::Adapters::FollowAdapter) }
  let(:block_adapter) { instance_double(Profile::Adapters::BlockAdapter) }

  before do
    allow(Profile::Adapters::FollowAdapter).to receive(:new).and_return(follow_adapter)
    allow(Profile::Adapters::BlockAdapter).to receive(:new).and_return(block_adapter)
  end
