...
module Identity
  module Grpc
    class Handler < ::Identity::V1::IdentityService::Service
      ...
      def send_sms(request, _call)
        send_sms_service.call(phone_number: request.phone_number)
        ::Identity::V1::SendSmsResponse.new(success: true)
      end

      def verify_sms(request, _call)
        result = verify_sms_service.call(phone_number: request.phone_number, code: request.code)
      ...
      def login(request, _call)
        result = login_service.call(phone_number: request.phone_number, password: request.password)
