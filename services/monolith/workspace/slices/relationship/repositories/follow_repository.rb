  ...
  module Repositories
    class FollowRepository < Relationship::DB::Repo
      def follow(cast_id:, guest_id:, status: "approved")
        existing = follows.where(cast_id: cast_id, guest_id: guest_id).one
        return { success: false, reason: :already_exists, status: existing.status } if existing

        follows.changeset(:create, cast_id: cast_id, guest_id: guest_id, status: status).commit
      ...
      def approve_follow(cast_id:, guest_id:)
        record = follows.where(cast_id: cast_id, guest_id: guest_id).one
      ...
      def list_pending_requests(cast_id:, limit: 100, cursor: nil)
        scope = follows.where(cast_id: cast_id, status: "pending")
      ...
      def pending_count(cast_id:)
        follows.where(cast_id: cast_id, status: "pending").count
      end

      def unfollow(cast_id:, guest_id:)
        deleted = follows.dataset.where(cast_id: cast_id, guest_id: guest_id).delete
        deleted > 0
      end

      def following?(cast_id:, guest_id:)
        follows.where(cast_id: cast_id, guest_id: guest_id, status: "approved").exist?
      end
      ...
      def list_following(guest_id:, limit: 100, cursor: nil)
        scope = follows.where(guest_id: guest_id, status: "approved")
      ...
      def get_follow_detail(cast_id:, guest_id:)
        record = follows.where(cast_id: cast_id, guest_id: guest_id, status: "approved").one
      ...
      def list_followers(cast_id:, blocked_guest_ids: [], limit: 20, cursor: nil)
        scope = follows.where(cast_id: cast_id, status: "approved")
