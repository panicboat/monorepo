  ...
  module UseCases
    module Posts
      class ListPublicPosts
        ...
        def call(limit: DEFAULT_LIMIT, cursor: nil, cast_user_id: nil, cast_user_ids: nil, exclude_cast_user_ids: nil)
          ...
          public_cast_user_ids = adapter.public_cast_ids

          # If filtering by specific cast_user_id, check if it's a public cast
          if cast_user_id && !public_cast_user_ids.include?(cast_user_id)
            return { posts: [], next_cursor: nil, has_more: false, authors: {} }
          end

          # Merge filters: only include posts from public casts
          effective_cast_user_ids = if cast_user_ids && !cast_user_ids.empty?
            cast_user_ids & public_cast_user_ids
          elsif cast_user_id
            [cast_user_id]
          else
            public_cast_user_ids
          end

          return { posts: [], next_cursor: nil, has_more: false, authors: {} } if effective_cast_user_ids.empty?

          posts = repo.list_all_visible(
            limit: limit,
            cursor: decoded_cursor,
            cast_user_ids: effective_cast_user_ids,
          ...
          author_cast_user_ids = pagination[:items].map(&:cast_user_id).uniq
        ...
        def load_authors(cast_user_ids)
          return {} if cast_user_ids.empty?

          adapter = Post::Adapters::CastAdapter.new
          adapter.find_by_cast_ids(cast_user_ids)
        end
