  ...
  module Repositories
    class LikeRepository < Post::DB::Repo
      def like(post_id:, guest_id:)
        existing = likes.where(post_id: post_id, guest_id: guest_id).one
        return if existing

        likes.changeset(:create, post_id: post_id, guest_id: guest_id).commit
      end

      def unlike(post_id:, guest_id:)
        likes.dataset.where(post_id: post_id, guest_id: guest_id).delete
      end

      def liked?(post_id:, guest_id:)
        likes.where(post_id: post_id, guest_id: guest_id).exist?
      end
