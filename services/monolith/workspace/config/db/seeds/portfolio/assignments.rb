...
# ジャンルを slug で取得
genre_ids = {}
db[:profile__genres].each { |g| genre_ids[g[:slug]] = g[:id] }
...
CAST_USER_IDS.each_with_index do |cast_user_id, idx|
  ...
  existing = db[:profile__cast_genres].where(cast_user_id: cast_user_id).count
  next if existing > 0
  ...
  slugs.each do |slug|
    ...
    db[:profile__cast_genres].insert(
...
# エリアを code で取得
area_ids = {}
db[:profile__areas].each { |a| area_ids[a[:code]] = a[:id] }
...
CAST_USER_IDS.each_with_index do |cast_user_id, idx|
  ...
  existing = db[:profile__cast_areas].where(cast_user_id: cast_user_id).count
  next if existing > 0
