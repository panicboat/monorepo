    ...
    class CreatePullRequest
      ...
      def execute(request, has_changes:)
        ...
        begin
          ...
          return Entities::PullRequestResult.failure(error_message: push_result.error_message) unless push_result.success?

          # Create pull request with labels
          pr_result = create_pull_request_with_labels(request)
          return Entities::PullRequestResult.failure(error_message: pr_result.error_message) unless pr_result.success?

          pull_request_url = pr_result.data[:pull_request_url]

          # Enable auto-merge with squash
          auto_merge_result = enable_auto_merge(pull_request_url)
          return Entities::PullRequestResult.failure(error_message: auto_merge_result.error_message) unless auto_merge_result.success?
      ...
      def push_feature_branch(request)
        begin
          ...
          Entities::Result.failure(error_message: "Failed to push feature branch: #{e.message}")
        end
      end
      ...
      def create_pull_request_with_labels(request)
        begin
          ...
          Entities::Result.failure(error_message: "Failed to create pull request: #{e.message}")
        end
      end
      ...
      def enable_auto_merge(pull_request_url)
        begin
          ...
          Entities::Result.failure(error_message: "Failed to enable auto-merge: #{e.message}")
        end
      end
