...
sequenceDiagram
    ...
    Matrix-->>DT: 2個のデプロイメントターゲット
    DT-->>GHA: デプロイマトリックス

    GHA->>TG: auth-service:environment 並列実行
    GHA->>TG: api-gateway:environment 並列実行
```
...
graph TD
    ...
    G --> H[auth-service → develop環境]
    G --> I[api-gateway → develop環境]
    G --> J[auth-service → staging環境]
    G --> K[api-gateway → staging環境]
    G --> L[auth-service → production環境]
    G --> M[api-gateway → production環境]
